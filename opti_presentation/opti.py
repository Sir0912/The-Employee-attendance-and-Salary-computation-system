...
@app.route("/api/admin/employees_list", methods=["GET"])
def employees_list():
    """Returns all employees for dropdown refresh."""
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT id_employee, name FROM opti ORDER BY name ASC")
    employees = cursor.fetchall()
    cursor.close(); conn.close()
